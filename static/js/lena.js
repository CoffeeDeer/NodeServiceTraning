...
/*global LenaJS:false */
LenaJS.histogram = function (pixels) {
...
LenaJS.contrastEnhance = function (pixels) {
    var count = 0, sum = 0
    for (var i = 0; i < pixels.data.length; i += 4) {
        sum += pixels.data[i]
        count ++
    ...
    for (var i = 0; i < pixels.data.length; i += 4) {
        for(var j = 0; j < 3; j++){
            ...
            temp = clamp(0, 255, temp)
            //temp = 1 / (1 + Math.pow(2.7182818284,(-1 * temp)))
            pixels.data[i+j] = temp
        }
    }
    return pixels
}

LenaJS.contrastEqualization = function (pixels) {
    var count = 0, sum = 0
    for (var i = 0; i < pixels.data.length; i += 4) {
        sum += pixels.data[i]
        count ++
